...
insert into cif.form_change(id, new_form_data, operation, change_status, form_data_schema_name, form_data_table_name, form_data_record_id, project_revision_id, json_schema_name, validation_errors)
  ...
  values
    ...
    (5,
     format('{"reportingRequirementId": 1, "adjustedGrossAmount": 99999, "adjustedNetAmount": 2}')::jsonb,
     'archive',
     'pending',
     'cif',
     'payment',
    ...
    (6,
     format('{"reportingRequirementId": 2, "adjustedGrossAmount": 99999, "adjustedNetAmount": 2}')::jsonb,
     'create',
     'pending',
     'cif',
     'payment',
...
select is(
  (
    ...
    ) select * from cif.project_revision_gross_payment_amount((select * from record))
  ),
  234.56,
  'Returns the sum of the adjusted gross amounts, only for the specified revision, non-archived form_changes'
);
