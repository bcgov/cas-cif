...
insert into cif.form_change(id, operation, form_data_schema_name, form_data_table_name, form_data_record_id, project_revision_id, json_schema_name, new_form_data)
  overriding system value
  values
    (1, 'create', 'cif', 'project', 1, 1, 'project', '{"operatorId": 1, "fundingStreamRfpId": 1, "projectStatusId": 1, "proposalReference": "001", "summary": "summary", "projectName": "project 1", "totalFundingRequest": 100, "comment": "some random comment"}'),
...
select results_eq(
  ...
  ) select operator_id, funding_stream_rfp_id, project_status_id, total_funding_request, proposal_reference, summary, project_name, comment from cif.form_change_as_project((select * from record))
  $$,
  $$
