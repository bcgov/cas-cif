-- Deploy cif:database_functions/verify_grants to pg
-- requires: schema_cif

begin;

-- Verifies permission grants for all columns in a table
create or replace function cif_private.verify_grant(
  operation text,
...
comment on function cif_private.verify_grant(text, text, text, text) is
...
comment on function cif_private.verify_grant(text, text, text, text[], text) is
