-- Deploy cif:database_functions/upsert_policy to pg
...
-- example: select cif_private.upsert_policy('test_policy', 'test_table', 'update', 'admin', 'true');
-- becomes: create policy test_policy on cif.test_table for update to admin using(true) with check(true);
create or replace function cif_private.upsert_policy(
  policy_name text,
  table_name text,
  operation text,
  role_name text,
...
-- example: select cif_private.upsert_policy('test_policy', 'test_table', 'update', 'admin', 'using(true)', 'with check(false)');
-- becomes: create policy test_policy on cif.test_table for update to admin using(true) with check(false);
create or replace function cif_private.upsert_policy(
  policy_name text,
  table_name text,
  operation text,
...
comment on function cif_private.upsert_policy(text, text, text, text, text, text) is
  'A generic function for creating or altering row-level security policies';
comment on function cif_private.upsert_policy(text, text, text, text, text, text, text) is
