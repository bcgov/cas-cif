-- Deploy cif:util/upsert_timestamp_columns to pg
-- requires: trigger_function/update_timestamps

begin;

create or replace function cif_private.upsert_timestamp_columns(
...
begin
  ...
  if not exists (select *
    ...
    trigger_string := concat(
      ...
      ' for each row execute procedure cif_private.update_timestamps()'
    );
