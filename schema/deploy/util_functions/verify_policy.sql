-- Deploy cif:database_functions/verify_policy to pg
-- requires: schema_cif

begin;

-- Verifies if a policy exists
create or replace function cif_private.verify_policy(
  operation text,
  policy_name text,
...
comment on function cif_private.verify_policy(text, text, text, text, text) is
