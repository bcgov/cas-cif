...
select
    case
      when (select exists (select get_form_status from cif.get_form_status($1.id, 'project_contact') where get_form_status = 'Incomplete'))
        then 'Incomplete'
      when (select exists (select get_form_status from cif.get_form_status($1.id, 'project_contact') where get_form_status = 'Attention Required'))
          then 'Attention Required'
      when (select count(distinct get_form_status) from cif.get_form_status($1.id, 'project_contact')) = 1
        and (select (select distinct get_form_status from cif.get_form_status($1.id, 'project_contact'))) = 'Complete'
          then 'Complete'
      else 'Not Started'
    end;
