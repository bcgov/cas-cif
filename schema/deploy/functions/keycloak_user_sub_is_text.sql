-- Deploy cif:functions/keycloak_user_sub_is_text to pg
-- requires: tables/cif_user
-- requires: types/keycloak_jwt

begin;

create or replace function cif_private.keycloak_user_sub_is_text()
returns void as
$function$
...
begin
  ...
  begin
    ...
    user_id := (select id from cif.cif_user where cif_user.session_sub = user_sub);
