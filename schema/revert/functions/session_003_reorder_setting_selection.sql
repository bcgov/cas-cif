-- Revert cif:functions/session_003_reorder_setting_selection from pg
...
begin
  if (coalesce(trim(_sub), '') = '') is not false then
    return null; -- ensure null, empty, and whitespace _sub claim is filtered out
  end if;
  return (
    select row (
      ...
      _sub, -- unique identifier can never be null
