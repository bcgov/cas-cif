...
--       -- create a general revision with all form data filled out
    ...
    insert into cif.form_change(
      ...
      (
        json_build_object(
          'cifUserId', 1,
          'projectId', (select form_data_record_id from cif.form_change where form_data_table_name='project' and project_revision_id=general_revision.id),
          'projectManagerLabelId', 1
          ),
        'create', 'cif', 'project_manager', 'pending', 'project_manager', general_revision.id);
      ...
      (
        ...
        'create', 'cif', 'project_contact', 'pending', 'project_contact', general_revision.id);



-- commit general revision
raise notice 'genrevid %',general_revision.id;

-- raise notice 'id %',(select project_revision_id from cif.form_change);
-- raise notice 'id %',(select change_status from cif.form_change);


  -- perform cif.commit_project_revision(general_revision.id) from cif.project_revision;
  select * from cif.commit_project_revision(general_revision.id) into coffee;
  -- select * from cif.commit_project_revision(3);
