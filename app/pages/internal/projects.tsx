...
function Projects({ preloadedQuery }: RelayProps<{}, projectsQuery>) {
  ...
  const approveStagedProject = async(id: string) => {
    ...
    await updateFormChangeMutation(preloadedQuery.environment, { id: id, formChangePatch: {changeStatus: 'saved'} } );
  };
  ...
  return (
    <DefaultLayout session={query.session} title="CIF Projects Management">
      <Grid.Row>
        ...
        <Grid.Col span={4}>
          <Input name='project-identifier' value={identifier} onInput={e => setIdentifier(e.target.value)}/>
        </Grid.Col>
      </Grid.Row>
      <Grid.Row>
        <Grid.Col span={2}>
        ...
        <Grid.Col span={4}>
          <Input name='project-description' value={description} onInput={e => setDescription(e.target.value)}/>
        </Grid.Col>
      </Grid.Row>
      <Grid.Row>
        <Grid.Col span={3}>
        <Button name='create-project' onClick={stageProject}>Create Project</Button>
        </Grid.Col>
      </Grid.Row>
      <br/>
      <hr/>
        ...
        {query.allFormChanges.edges.map(({ node }) => {
          ...
          return (
            <Card title={cardTitle} key={node.id}>
              ...
              <Grid.Row>
                <Grid.Col span={3}>
                  <Button onClick={() => approveStagedProject(node.id)}>Approve</Button>
                </Grid.Col>
                <Grid.Col span={3}>
                  <Button onClick={() => rejectStagedProject(node.id)}>Reject</Button>
                </Grid.Col>
              </Grid.Row>
            </Card>
          )
